...
import { SampleControllerApi } from '@c4-soft/poc-api';
...
export class SettingsScreen implements OnInit {
  ...
  constructor(
    private sampleApi: SampleControllerApi,
  ...
  ngOnInit() {
      ...
      this.storage.get('solutionsPath').then(
        (p) => {
          this.sampleApiPathCtrl.patchValue(
            p || this.sampleApi.configuration.basePath
          )
          if (p) {
            this.sampleApi.configuration.basePath = p;
          }
        },
        (error) => {
          console.error(error);
          this.sampleApiPathCtrl.patchValue(
            this.sampleApi.configuration.basePath
          );
        }
      );
    });
    this.sampleApiPathCtrl.valueChanges.subscribe((newPath) => {
      if (this.sampleApiPathCtrl.valid) {
        ...
        this.sampleApi.configuration.basePath = newPath;
      }
    });
  }
}